<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="学生学号" prop="xueshengxuehao">
          <el-input v-model="ruleForm.xueshengxuehao" 
              placeholder="学生学号" clearable  :readonly="ro.xueshengxuehao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="学生学号" prop="xueshengxuehao">
              <el-input v-model="ruleForm.xueshengxuehao" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="任务完成率" prop="renwuwanchenglv">
          <el-select :disabled="ro.renwuwanchenglv" v-model="ruleForm.renwuwanchenglv" placeholder="请选择任务完成率">
            <el-option
                v-for="(item,index) in renwuwanchenglvOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="任务完成率" prop="renwuwanchenglv">
	      <el-input v-model="ruleForm.renwuwanchenglv"
                placeholder="任务完成率" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="工作积极性" prop="gongzuojijixing">
          <el-select :disabled="ro.gongzuojijixing" v-model="ruleForm.gongzuojijixing" placeholder="请选择工作积极性">
            <el-option
                v-for="(item,index) in gongzuojijixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="工作积极性" prop="gongzuojijixing">
	      <el-input v-model="ruleForm.gongzuojijixing"
                placeholder="工作积极性" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="考核结果" prop="kaohejieguo">
          <el-select :disabled="ro.kaohejieguo" v-model="ruleForm.kaohejieguo" placeholder="请选择考核结果">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="考核结果" prop="kaohejieguo">
	      <el-input v-model="ruleForm.kaohejieguo"
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="工作证明" prop="gongzuozhengming">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.gongzuozhengming" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.gongzuozhengming" label="工作证明" prop="gongzuozhengming">
                    <span v-html="ruleForm.gongzuozhengming"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	gongzuotaidu : false,
	renwuwanchenglv : false,
	gongzuojijixing : false,
	kaohejieguo : false,
	danweimingcheng : false,
	tijiaoshijian : false,
	gongzuozhengming : false,
      },
      ruleForm: {
        xueshengxuehao: '',
        xueshengxingming: '',
        ...
        gongzuozhengming: '',
          ...
          gongzuotaiduOptions: [],
          renwuwanchenglvOptions: [],
          gongzuojijixingOptions: [],
          kaohejieguoOptions: [],
      rules: {
          xueshengxuehao: [
          ],
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gongzuotaidu = true;
          ...
          if(o=='renwuwanchenglv'){
            this.ruleForm.renwuwanchenglv = obj[o];
	    this.ro.renwuwanchenglv = true;
            continue;
          }
          if(o=='gongzuojijixing'){
            this.ruleForm.gongzuojijixing = obj[o];
	    this.ro.gongzuojijixing = true;
            continue;
          }
          if(o=='kaohejieguo'){
	    ...
	    this.ro.tijiaoshijian = true;
          ...
          if(o=='gongzuozhengming'){
            this.ruleForm.gongzuozhengming = obj[o];
	    this.ro.gongzuozhengming = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.danweimingcheng!=''&&json.danweimingcheng){
          ...
          this.$message.error(data.msg);
            ...
            this.gongzuotaiduOptions = "优秀,良好,及格,不及格".split(',')
            this.renwuwanchenglvOptions = "优秀,良好,及格,不及格".split(',')
            this.gongzuojijixingOptions = "优秀,良好,及格,不及格".split(',')
            this.kaohejieguoOptions = "优秀,良好,及格,不及格".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `danweichengji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.gongzuozhengming = this.ruleForm.gongzuozhengming.replace(reg,'../../../springbootrij2p/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `danweichengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.danweichengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
