<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="实习评分" prop="shixipingfen">
          <el-input v-model="ruleForm.shixipingfen" 
              placeholder="实习评分" clearable  :readonly="ro.shixipingfen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="实习评分" prop="shixipingfen">
              <el-input v-model="ruleForm.shixipingfen" 
                placeholder="实习评分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="批改时间" prop="pigaishijian">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.pigaishijian" label="批改时间" prop="pigaishijian">
              <el-input v-model="ruleForm.pigaishijian" 
                placeholder="批改时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="实习评价" prop="shixipingjia">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shixipingjia" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shixipingjia" label="实习评价" prop="shixipingjia">
                    <span v-html="ruleForm.shixipingjia"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.pigaishijian = true;
          ...
          if(o=='shixipingjia'){
            this.ruleForm.shixipingjia = obj[o];
	    this.ro.shixipingjia = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.shixipingjia = this.ruleForm.shixipingjia.replace(reg,'../../../springbootrij2p/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaoshipingfen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaoshipingfenCrossAddOrUpdateFlag = false;
						 this.parent.search();
