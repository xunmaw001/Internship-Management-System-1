<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="单位名称" prop="danweimingcheng">
          <el-select :disabled="ro.danweimingcheng" @change="danweimingchengChange" v-model="ruleForm.danweimingcheng" placeholder="请选择单位名称">
            <el-option
                v-for="(item,index) in danweimingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.danweimingcheng" label="单位名称" prop="danweimingcheng">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="单位地址" prop="danweidizhi">
          <el-input v-model="ruleForm.danweidizhi" 
              placeholder="单位地址" clearable  :readonly="ro.danweidizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="单位地址" prop="danweidizhi">
              <el-input v-model="ruleForm.danweidizhi" 
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="实习时间" prop="shixishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.shixishijian" 
                type="datetime"
                :readonly="ro.shixishijian"
                placeholder="实习时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shixishijian" label="实习时间" prop="shixishijian">
              <el-input v-model="ruleForm.shixishijian" 
                placeholder="实习时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.shixibaogao" label="实习报告" prop="shixibaogao">
          <file-upload
          tip="点击上传实习报告"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shixibaogao" label="实习报告" prop="shixibaogao">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.shixibaogao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.shixishijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.danweimingcheng = true;
          ...
          if(o=='danweidizhi'){
            this.ruleForm.danweidizhi = obj[o];
	    this.ro.danweidizhi = true;
            continue;
          }
          if(o=='xueshengxuehao'){
	    ...
	    this.ro.jiaoshixingming = true;
          ...
          if(o=='shixishijian'){
            this.ruleForm.shixishijian = obj[o];
	    this.ro.shixishijian = true;
            continue;
          }
          if(o=='shixibaogao'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.xueshengxuehao!=''&&json.xueshengxuehao){
		...
		if(json.yuanxi!=''&&json.yuanxi){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shixidanwei/danweimingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.danweimingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    danweimingchengChange () {
      this.$http({
        url: `follow/shixidanwei/danweimingcheng?columnValue=`+ this.ruleForm.danweimingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.danweidizhi){
                this.ruleForm.danweidizhi = data.data.danweidizhi
              }
	...
	if(this.ruleForm.shixibaogao!=null) {
		this.ruleForm.shixibaogao = this.ruleForm.shixibaogao.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shixizuoye/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shixizuoyeCrossAddOrUpdateFlag = false;
						 this.parent.search();
	...
	this.ruleForm.shixibaogao = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
